...
- БЕЗ ФОНА В ЭКСПОРТЕ. Фон/декор/стол/пропсы входят в BACKDROP_SLOTS и
  вырезаются из ВСЕХ кадрирований — скачиваемый файл содержит только
  персонажа на прозрачности/хромакее.
  ВАЖНО: в defaults архетипа фон ОСТАЁТСЯ (p_bg_wall_flat и т.д.) —
  он нужен как декорация превью в студии и чате, где рисуется вся сцена
  1600x900 без кропа. Выключать его в defaults НЕЛЬЗЯ: сцена станет
  прозрачной и персонаж повиснет в пустоте (проверено: 86% пустого кадра).
  Разделение проверяется тестом: превью содержит hex стены, экспорт — нет.
...
## K2 — ВЫПОЛНЕНО (в патче, не переделывать)

Сделано: animations.json грузится клиентом (`ClientAssets.animations`);
`AvatarPlayer(svg, clips, { loopMs })` + `setLoopMs()` — зацикливание
времени, иначе idle с endMs=7200 замирал после первого круга;
`AvatarStage` принимает `loopMs` и меняет таймлайн без пересборки DOM;
стор получил `animPreset`/`setAnimPreset`; в StudioPanel секция «Анимация»
(10 чипов) перед палитрой; кнопка «Скопировать код аватара» (base64 конфига).
`next build` → Compiled successfully.

### Историческая справка по реализации K2 (для понимания решений)
